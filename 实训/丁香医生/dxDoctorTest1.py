...
def get_info_text(html):
    ...
    info_list = list()
    print('开始写入....')
    with open('dxInfos1.csv', 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        for province in text['window.getAreaStat']:
            ...
            writer.writerow(['provinceName', 'currentConfirm', 'confirm', 'death', 'cure'])
            writer.writerow(provinceRow)
            info_list.append(provinceRow)
            if province['cities']:
                writer.writerow(['cityName', 'currentConfirm', 'confirm', 'death', 'cure'])
...
def plot(infos):
    ...
    # bar3.set_global_opts(legend_opts=opts.LegendOpts(pos_top='bottom'))
    bar3.set_global_opts(title_opts=opts.TitleOpts(title='确诊人数超1000的省份柱状图', pos_top='bottom'))
    bar4.add_xaxis(['currentConfirm', 'confirm', 'death', 'cure'])
    bar4.set_global_opts(title_opts=opts.TitleOpts(title='确诊人数小于1000的省份柱状图', pos_top='bottom'))
    for i in infos:
        if i[2] > 10000:
